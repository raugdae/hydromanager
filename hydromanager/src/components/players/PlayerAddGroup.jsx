...
function PlayerAddGroup ({attendeeid,onCancel}) {
  ...
  const fetchGroupList = async () => {
    ...
    setGroupList(response.data.sort((a, b) => 
...
const handleConfirmAdd = async() => {
  ...
  console.log("pouet pouet");
    ...
    return (<div className="absolute inset-0 z-10 grid grid-cols-4 bg-zinc-200 flex-1 justify-items-center">
      ...
      <div className='col-start-3 row-start-3'>
        <ConfirmButton disabled={isSaving} onClick={() => handleConfirmAdd()}>{isSaving?"Envoi en cours...":"Confirmer"}</ConfirmButton>
      </div>
    </div>)
